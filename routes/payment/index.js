...
const givePurchase = (billId,response) => {
  getOrder(billId).then(
    (order) => {
      const { id, product_id, price, status, player, email } = order[0];
...
payment.post("/status/qiwi", (request, response) => {
  if (request.body) {
    ...
    const { billId, siteId, amount, status } = bill;
...
payment.post("/status/payok", (request, response) => {
  if (request.body) {
    ...
    var string = `${PAYOK_CONF.SECRET_KEY}|${desc}|${currency}|${shop}|${billId}|${amount}`;
    var controlHash = forge.md.md5
      .create()
      .update(string, "utf8")
      .digest()
      .toHex();
...
payment.post("/status/anypay", (request, response) => {
  if (request.body) {
    ...
    var string = `${currency}:${amount}:${billId}:${ANYPAY_CONF.PROJECT_ID}:${status}:${ANYPAY_CONF.SECRET_KEY}`;
    var controlHash = forge.md.sha256
      .create()
      .update(string, "utf8")
      .digest()
      .toHex();
    ...
    registerOrder(
      ...
      switch (order.payment_type) {
        case "qiwi":
          ...
          var link = QIWI_API.createPaymentForm(params);
        ...
        case "anypay":
          ...
          var link = `https://anypay.io/merchant?merchant_id=${
            params.merchant_id
        ...
        case "mobile":
          var params = {
            ...
            sign: function () {
              var string = `${this.amount}|${this.payment}|${this.shop}|${this.currency}|${this.desc}|${PAYOK_CONF.SECRET_KEY}`;
              var sign = forge.md.md5
                .create()
                .update(string, "utf8")
                .digest()
                .toHex();
          ...
          var link = `https://payok.io/pay?amount=${params.amount}&payment=${
            params.payment
