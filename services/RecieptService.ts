...
export class RecieptService {
  ...
  auth(login: string, password: string): Promise<any> {
    ...
    this.authPromise = fetch(this.apiUrl + '/auth/lkfl', {
      ...
      if (!response.refreshToken) {
        console.error((response.message || 'Не получилось авторизоваться'));
        return;
      }
  ...
  async getToken(): Promise<any> {
    ...
    if (!this.authPromise) {
      console.error('Необходимо сначала авторизоваться');
      return;
    }
   ...
   */
  async call(endpoint: string, payload?: object, method: string = 'GET'): Promise<any> {
    if (payload) {
      method = 'POST';
    }
   ...
   */
  async addIncome(date = new Date(), name: string, quantity = 1, amount: number): Promise<any> {
