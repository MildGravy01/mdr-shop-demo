...
if( basename(__FILE__) == basename($_SERVER["SCRIPT_FILENAME"]) ) {
	// Don't adjust the error reporting if we are an include file
	 // TODO not here - this is set in index.php
...
function myHandler($level, $message, $file, $line, $context) {
    ...
    writeCommandLog($file." ".$line." ".$message);
    return true;
}
set_error_handler('myHandler', E_ALL);

function writeCommandLog($log)
{
  $log_file = fopen(__DIR__ . '/skin_log.txt', 'a+');
  fwrite($log_file, $log.PHP_EOL);
  fclose($log_file);
}
	...
	class render3DPlayer {
		...
		private function getSkinURLViaUUIDFromMDR($UUID) {
			$true_uuid= grabGetValue('user');
			$db = new mysqli("94.250.254.146:3306", "skin", "b8xHULy", "mdr_server");
			$nickname = mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `playerdata` WHERE `uuid`='".$true_uuid."'"))['name'];
			writeCommandLog("NickName is: ".$nickname." UUID: ".$true_uuid);
			$skin = mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `Skins` WHERE `Nick`='".$nickname."'"))['Value'];
			writeCommandLog("Skin is: ".$skin);
			if($skin == null) {
				$nick = mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `Players` WHERE `Nick`='".$nickname."'"))["Skin"];
				$skin = mysqli_fetch_assoc(mysqli_query($db,"SELECT * FROM `Skins` WHERE `Nick`='".$nick."'"))["Value"];
			}
		...
		private function getSkinURL() {
			...
			if($isUUID !== false) {
				$result = $this->getSkinURLViaUUIDFromMDR($isUUID);
				return $result;
			}
			else{
				...
				$result = $this->getSkinURLViaUUIDFromMDR($profileContentArray["id"]);
				return $result;
			}
			return false;
		}
		...
		public function get3DRender($output = 'return') {
			...
			switch($this->format) {
				...
				default:
					...
					header('X-Storage-Type: downloaded');
					header('Access-Control-Allow-Origin: *');
					imagepng($result);
					imagedestroy($result);
					break;
