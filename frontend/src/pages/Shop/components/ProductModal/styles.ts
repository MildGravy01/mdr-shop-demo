...
export const ImageHolder = styled.div`
  ...
  align-self: center;
`;
export const ProductImage = styled.img<{countable?: boolean}>`
  ${({countable}) => {
    if (countable) {
      return css`
        width: 100%;
...
export const InfoHolder = styled.div<{height?: string}>`
  position: inherit;
  display: flex;
  flex-wrap: nowrap;
  font-size: 18px;
...
export const ModalBody = styled.div<{height: number | string; margin?: number | string}>`
  border-radius: 10px;
